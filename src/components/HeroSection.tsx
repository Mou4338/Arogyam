...
export default function HeroSection() {
  ...
  const healthFeatures = [
    {
      ...
      description:
        "Receive tailored health advice and recommendations based on your unique profile and interactions with the AI chatbot.",
    },
    {
      icon: Zap,
      title: "Rapid Emergency Response",
      description:
        "Access critical emergency services and transportation booking with unparalleled speed and efficiency during urgent situations.",
    },
    {
      icon: CalendarCheck,
      title: "Seamless Virtual Care",
      description:
        "Connect with certified medical professionals through secure video consultations, making healthcare accessible from anywhere.",
    },
    {
      icon: ClipboardList,
      title: "Comprehensive Health Management",
      description:
        "Manage appointments, track reminders, and stay updated on public health alerts—all from one intuitive platform.",
    },
  ];
  ...
  return (
      ...
      <section className="relative flex items-center justify-center min-h-[60vh] bg-[url('/bg-hero.png')] bg-cover bg-center">
        ...
        <div className="relative z-10 flex flex-col items-center text-center px-4 py-16">
          ...
          <div className="flex gap-4">
            <button className="bg-[#64bcae] hover:cursor-pointer text-white font-heading font-semibold px-6 py-2 rounded hover:bg-[#132d2e] transition-colors duration-200">
              Find a Hospital
            </button>
            <button className=" hover:bg-[#132d2e] hover:cursor-pointer bg-white text-black border border-gray-300 font-heading px-6 py-2 rounded hover:bg-gray-100 transition-colors duration-200">
              Explore Services
            </button>
          </div>
        </div>
      ...
      <section className="bg-slate-100 py-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto px-4">
          {services.map((service, index) => (
            <div key={index} className="hover:transform-y-1 transition:transform duration:200 bg-[#3f8578] shadow-lg rounded-lg p-6 flex flex-col items-start">
    ...
    <section className="py-6 flex flex-col md:flex-row items-center justify-between gap-8 px-4 bg-[#3f8578]">
      <div className="flex-1 flex flex-col rounded shadow-lg px-4 py-2 max-w-60%">
        ...
        <div className="grid grid-cols-1 gap-6">
          {healthFeatures.map((feature, index) => (
            <div
              key={index}
              className="flex items-start gap-4 bg-[#132d2e] shadow-lg rounded-lg p-4 hover:shadow-2xl transition"
            >
